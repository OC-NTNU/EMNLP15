...
% For this purpose, the pattern contains an assignment of the name d1 to the node directly dominating the lemma \emph{variability} % ( \verb|/NN/=d1 < variability|)), allowing a corresponding tree operation to delete this node.
%, which is implemented using the Tsurgeon counterpart of Tregex.
...
%False positives also occur because of different semantics of the same pattern, e.g. \emph{change in western Europe} is unlikely to mean literally that Europe is changing, neither does \emph{changes in less than a few thousand years} imply that past years are changing.
%At the same time, false negatives are beyond the possibilities of pattern matching.
%For instance, ... 
%Also, 
%Variation may also be entailed rather than explicitly stated.
%For example,  \emph{ocean acidification} entails increasing acidity of ocean water and  \emph{Arctic warming} entails increasing %temperature in the Arctic region.
%This is closely related to recognising textual entailment \cite{AndroutsopoulosMalakasiotis:2010,DaganGlickmanMagnini:2006}, requiring inference in combination with domain knowledge.
...
%
%Variables can be recursive or embedded, expressing ``a change of a change''.  
...
%The current visualisation is insufficient in these cases, as HTML browsers cannot render a link in a link.
%
...
%In addition, variables often contain references to previously mentioned entities -- anaforic \emph{it} being the ultimate example of this -- suggesting a need for co-reference resolution \cite{Miwa01072012}.
